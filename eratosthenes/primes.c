...
uint32_t _reset_lsb(uint32_t x) { \
    __asm__ volatile ( \
    "blsr %0, %0 \n\t" \
    : "=r"(x) \
    : "0"(x) );
	...
	uint32_t start_count, uint64_t *primes)
{
	...
	range = sdata->numlinebytes / SOE_THREADS;
	range -= (range % 32);
	lastid = 0;
    ...
    if (SOE_THREADS > 1)
    {
        ...
        if (sdata->sieve_range)
        {
            ...
            memchunk = (sdata->orig_hlimit - sdata->orig_llimit) / SOE_THREADS + SOE_THREADS;
        ...
        else
        {
            ...
            uint64_t tmphi;
            uint64_t chunk = 8 * sdata->numlinebytes / SOE_THREADS;
            ...
            for (i = 0; i < SOE_THREADS; i++)
            {
                ...
                if (SOE_VFLAG > 2)
                {
                    printf("allocating temporary space for %lu primes between %lu and %lu\n",
    ...
    tpool_data = tpool_setup(SOE_THREADS, NULL, NULL, NULL,
	...
	if (SOE_THREADS > 1)
	{
		...
		for (j = 0; j < SOE_THREADS; j++)
		{
			...
			if (SOE_VFLAG > 2)
				printf("adding %lu primes found in thread %d\n", t->linecount, j);
    ...
    uint32_t pcount, uint64_t *primes, uint64_t byte_offset)
{
    ...
    if (1)
    {
        ...
        __m256i vprodN = _mm256_set1_epi32((uint32_t)prodN);
        __m256i vbitoffset;
        __m256i vbits, vbits2;
        __m256i vtmp2, vtmp3, vtmp4;
#if defined(__GNUC__)
        __attribute__((aligned(64))) uint32_t tmpstore[8];
        __attribute__((aligned(64))) uint32_t tmpstore2[8];
#else
        __declspec(align(64)) uint32_t tmpstore[8];
        __declspec(align(64)) uint32_t tmpstore2[8];
#endif
        int i;
        int poffset1[8];
        //int poffset2[8];
        ...
        vbits = _mm256_loadu_si256((__m256i *)(&lines[0][byte_offset]));
		vtmp4 = _mm256_loadu_si256((__m256i *)(&lines[1][byte_offset]));
        //vtmp4 = _mm256_loadu_si256((__m256i *)(&lines[0][byte_offset + sdata->numlinebytes]));
	...
	uint32_t pcount, uint64_t *primes, uint64_t byte_offset)
{
	...
	memset(cache_word, 0, 64 * sizeof(uint64_t));
	for (current_line = 0; current_line < nc; current_line++)
	{
		...
		uint32_t line_div8 = current_line >> 3;
		uint32_t line_mod8 = current_line & 7;
		cache_word[line_div8] |= ((uint64_t)lines[current_line][byte_offset] << (line_mod8 << 3));
		cache_word[8 + line_div8] |= ((uint64_t)lines[current_line][byte_offset+1] << (line_mod8 << 3));
		cache_word[16 + line_div8] |= ((uint64_t)lines[current_line][byte_offset+2] << (line_mod8 << 3));
		cache_word[24 + line_div8] |= ((uint64_t)lines[current_line][byte_offset+3] << (line_mod8 << 3));
		cache_word[32 + line_div8] |= ((uint64_t)lines[current_line][byte_offset+4] << (line_mod8 << 3));
		cache_word[40 + line_div8] |= ((uint64_t)lines[current_line][byte_offset+5] << (line_mod8 << 3));
		cache_word[48 + line_div8] |= ((uint64_t)lines[current_line][byte_offset+6] << (line_mod8 << 3));
		cache_word[56 + line_div8] |= ((uint64_t)lines[current_line][byte_offset+7] << (line_mod8 << 3));
	}
	...
	for (b = 0; b < 64; b++)
	{
		for (current_line = 0; current_line < nc; current_line++)
		{
			...
			//if (cache_word & ((uint64_t)nmasks[b] << (current_line << 3)))
    ...
    uint32_t pcount, uint64_t *primes, uint64_t byte_offset)
{
    ...
    if ((nc == 8) && (lowlimit == 0))
    {
        ...
        // first line's byte offset, so the indices are all just multiples
        // of the length of a line.
        __m256i vtmp1 = _mm256_set1_epi32((uint32_t)sdata->numlinebytes);
        __m256i vlinenums = _mm256_setr_epi32(0, 1, 2, 3, 4, 5, 6, 7);
        __m256i voffsets = _mm256_mullo_epi32(vtmp1, vlinenums);
        __m256i vmasks = _mm256_set1_epi32(0x1);
        __m256i vprodN = _mm256_set1_epi32((uint32_t)prodN);
        __m256i vbitoffset = _mm256_set1_epi32((uint32_t)byte_offset << 3);
