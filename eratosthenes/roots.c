...
void getRoots(soe_staticdata_t *sdata, thread_soedata_t *thread_data)
{
    ...
    range = (sdata->pboundi - sdata->bucket_start_id) / SOE_THREADS;
    ...
    if (range > 0)
    {
        ...
        if (thread_data[SOE_THREADS - 1].stopid != sdata->pboundi)
        {
            thread_data[SOE_THREADS - 1].stopid = sdata->pboundi;
        }
	...
	for (; i < sdata->pboundi;i++)
	{
		...
		uint32_t startclass;
		uint64_t startbit;
		uint32_t rclass, bnum, rclassid;
		uint32_t index = i - sdata->inplace_start_id;
		int a;
		...
		starthit += ((uint64_t)sdata->sieve_p[i] * (uint64_t)(startclass >> 8));
		...
		startbit = (starthit - sdata->lowlimit - (uint64_t)startclass) / (uint64_t)prodN;

		// sanity check
		if (((starthit - sdata->lowlimit - (uint64_t)startclass) % (uint64_t)prodN) != 0)
